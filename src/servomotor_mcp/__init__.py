...
straight to the hardware, with no software clamping. Nothing is hardcoded: serial ports
are enumerated live, motors are auto-detected on the bus, and every command in the
``servomotor`` library's catalog is exposed as an MCP tool. Ships with a mock backend so
it runs with no hardware.
